...
Explain the relationship between the caption and the meme text in a short paragraph. Identify if it's humorous, exaggerated, sarcastic, or ironic.
"""
    try:
...
# Stage 2: KeyBERT + Transformer + Multi-task MLP
kw_model = KeyBERT()
...
class MultiTaskClassifier(nn.Module):
    def __init__(self, hidden_size=768):
        ...
        self.sentiment = nn.Linear(hidden_size, 3)    # pos/neg/neutral
        self.emotion = nn.Linear(hidden_size, 5)      # emotion classes
...
interface = gr.Interface(
    ...
    outputs=[
        ...
        gr.Textbox(label="Extracted Meme Text"),
        gr.Textbox(label="Meme Summary"),
        gr.Textbox(label="Sentiment Analysis")
    ],
    title="Two-Stage Meme Interpretation Pipeline",
    description="Upload a meme to generate its caption, extract text, explain it, and classify its meaning (humor, sarcasm, sentiment, etc.)."
)
