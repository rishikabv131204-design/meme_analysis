...
# -----------------------------
# 🎞️ Process Video Function
# -----------------------------
def extract_key_frame(video_path):
    cap = cv2.VideoCapture(video_path)
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    key_frame_index = total_frames // 2  # middle frame
...
def process_video(video):
    ...
    if frame is None:
        return "Could not extract frame.", "N/A", "N/A", "N/A"

    caption = generate_caption(frame)
    frame_np = np.array(frame)
    ocr_result = ocr_reader.readtext(frame_np)
...
interface = gr.Interface(
    ...
    description="Upload a meme video — this model extracts a key frame, describes it, explains the humor, and classifies its tone."
)
